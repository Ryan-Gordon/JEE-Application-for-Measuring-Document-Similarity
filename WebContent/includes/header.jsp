<html>
	<head>
		<title>2017 Assignment -  A JEE Application for Measuring Document Similarity</title>
		<link rel="stylesheet" href="includes/basic.css">
			<!-- load MUI -->
    <link href="//cdn.muicss.com/mui-0.9.35/css/mui.min.css" rel="stylesheet" type="text/css" />
    <link href=" http://code.ionicframework.com/ionicons/2.0.1/css/ionicons.min.css" rel="stylesheet" type="text/css" />
   
    <script src="//cdn.muicss.com/mui-0.9.35/js/mui.min.js"></script>
  </head>
  <body>
    <!-- example content -->
    <header class="mui-appbar mui--z1">
      <div class="mui-container">
        <table>
          <tr class="mui--appbar-height">
          	<td align="mui--text-right">
              <ul class="mui-list--inline mui--text-body2">
                <li><a href="https://github.com/Ryan-Gordon/JEE-Application-for-Measuring-Document-Similarity"><i class="icon ion-social-github"></i>Github</a></li>
              </ul>
            </td>
            <td class="mui--text-title">Measuring Document Similarity</td>
            
          </tr>
        </table>
      </div>
    </header>
       <div id="content-wrapper" class="mui--text-center">
      <div class="mui--appbar-height"></div>